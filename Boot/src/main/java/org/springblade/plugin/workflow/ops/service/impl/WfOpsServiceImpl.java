...
public class WfOpsServiceImpl implements IWfOpsService {
	...
	public IPage<WfOps> list(WfOps ops, Query query) {
		...
		if (ObjectUtil.isNotEmpty(ops.getIsSuspended())) { // 是否挂起
			if (ops.getIsSuspended()) {
				taskInstanceQuery.suspended();
			} else {
				taskInstanceQuery.active();
			}
		}

		WfSearchUtil.buildSearchQuery(taskInstanceQuery, ops);
		...
		if (count > 0) {
			...
			taskList.forEach(task -> {
				...
				wfOps.setIsMultiInstance(wfTaskService.isMultiInstance(task.getTaskDefinitionKey(), task.getProcessDefinitionId()));
				if (StringUtil.isNotBlank(task.getAssignee())) {
					WfUser user = WfUserCache.getUser(Long.valueOf(task.getAssignee()));
					if (user != null) {
					...
					if (ObjectUtil.isNotEmpty(userList)) {
						wfOps.setAssignee(userList.stream().map(WfUser::getName).collect(Collectors.joining("/")));
					}
				}
				// Variables
				Object applyUsername = runtimeService.getVariable(task.getProcessInstanceId(), WfProcessConstant.TASK_VARIABLE_APPLY_USER_NAME);
				if (ObjectUtil.isNotEmpty(applyUsername)) {
					wfOps.setStartUsername(applyUsername.toString());
				}
				Object sn = runtimeService.getVariable(task.getProcessInstanceId(), WfProcessConstant.TASK_VARIABLE_SN);
				if (ObjectUtil.isNotEmpty(sn)) {
					wfOps.setSerialNumber(sn.toString());
				}
				// ProcessInstance
				ProcessInstance processInstance = runtimeService.createProcessInstanceQuery()
					.processInstanceId(task.getProcessInstanceId())
					.singleResult();
				wfOps.setProcessDefinitionName(processInstance.getName());
				// 扩展属性
	...
	public IPage<WfOps> processList(WfOps ops, Query query) {
		...
		if (count > 0) {
			processIPage.getRecords().forEach(p -> {
				WfOps wfOps = Objects.requireNonNull(BeanUtil.copyProperties(p, WfOps.class));
				Future<List<WfProcess>> historyFlowList = wfProcessService.historyFlowList(p.getProcessInstanceId(), null, null, WfTaskUtil.getTokenUser());
...
// 				...
	...
	public void completeTask(WfOps ops) {
		...
		if (StringUtil.isBlank(ops.getComment())) {
			WfUser user = WfUserCache.getUser(Long.valueOf(WfTaskUtil.getTaskUser()));
	...
	public void changeTaskAssignee(WfOps ops) {
		...
		for (String id : taskIds) {
			...
			try {
				WfUser fromUser = WfUserCache.getUser(task.getAssignee() == null ? Long.valueOf(WfTaskUtil.getTaskUser()) : Long.valueOf(task.getAssignee()));
				WfUser toUser = WfUserCache.getUser(Long.valueOf(assignee));
	...
	@Override
	public void deleteProcessInstance(WfOps ops) {
		String[] arr = ops.getProcessInstanceId().split(",");
		for (String processInsId : arr) {
			ProcessInstance processInstance = runtimeService.createProcessInstanceQuery()
			...
			if (processInstance != null) {
				...
				list.forEach(t -> taskService.addComment(t.getId(), processInsId, WfProcessConstant.COMMENT_TYPE_DELETE_PROCESS, finalComment));
				runtimeService.deleteProcessInstance(processInsId, comment);

				// 删除抄送
				wfCopyService.remove(new LambdaQueryWrapper<WfCopy>()
					.eq(WfCopy::getProcessId, processInsId));
			}
			if ("force".equals(ops.getStatus())) { // 物理删除
