...
public class WfSerialServiceImpl extends BaseServiceImpl<WfSerialMapper, WfSerial> implements IWfSerialService {
	...
	public void create(BpmnModel model, String deploymentId) {
		...
		if (attributes != null) {
			List<ExtensionAttribute> useExistSNs = attributes.get("useExistSN");
			if (useExistSNs != null && !useExistSNs.isEmpty()) {
				String useExistSN = useExistSNs.get(0).getValue();
				if ("true".equals(useExistSN)) {
					return;
				}
			}
			...
			if (processDefinition != null && processDefinition.getVersion() > 1) {
	...
	public String getNextSN(BpmnModel model, String deploymentId) {
		WfSerial serial = null;
		...
		if (attributes != null) {
			...
			if (useExistSNs != null && !useExistSNs.isEmpty()) {
				...
				if ("true".equals(useExistSN)) {
					...
					String prefix = attributes.get("prefix").get(0).getValue();
					List<WfSerial> list = this.list(new LambdaQueryWrapper<WfSerial>()
						.eq(WfSerial::getName, name)
						.eq(WfSerial::getPrefix, prefix)
						.orderByDesc(WfSerial::getCreateTime));
					if (list != null && !list.isEmpty()) {
						serial = list.get(0);
					}
				} else {
					serial = this.getOne(new LambdaQueryWrapper<WfSerial>().eq(WfSerial::getDeploymentId, deploymentId));
				}
			} else {
				serial = this.getOne(new LambdaQueryWrapper<WfSerial>().eq(WfSerial::getDeploymentId, deploymentId));
			}
