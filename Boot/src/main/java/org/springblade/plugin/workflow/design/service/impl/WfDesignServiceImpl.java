...
public class WfDesignServiceImpl extends ServiceImpl<WfModelMapper, WfModel> implements IWfDesignService {
	...
	private final WfFormVariableMapper wfFormVariableMapper;
	...
	public IPage<WfProcessDef> deploymentPage(WfProcessDef process, Query query) {
		...
		if (ObjectUtil.isNotEmpty(process.getScope()) && process.getScope()) {
			List<String> scopeAll = new ArrayList<>(Collections.singletonList("WF_ALL"));
			scopeAll.addAll(Arrays.asList(WfTaskUtil.getCandidateGroup().split(",")));
			processDefinitionQuery.startableByUserOrGroups(WfTaskUtil.getTaskUser(), scopeAll);
		}
			...
			processDefinitionList.forEach(processDefinition -> {
				...
				try {
					WfModel model = this.getOne(new LambdaQueryWrapper<WfModel>()
						...
						.eq(WfModel::getTenantId, processDefinition.getTenantId()));
					processDef.setIcon(model.getIcon());
				} catch (Exception ignore) {
	...
	public void deleteDeployment(String deploymentId) {
		...
		wfCopyMapper.delete(new QueryWrapper<>(copy));

		// 删除流程表单变量
		wfFormVariableMapper.delete(new LambdaQueryWrapper<WfFormVariable>()
				.eq(WfFormVariable::getDeploymentId, deploymentId));

		// 删除数据表 - 低代码方式
		List<WfDeploymentForm> list = wfDeploymentFormMapper.selectList(new LambdaQueryWrapper<WfDeploymentForm>()
				.eq(WfDeploymentForm::getDeploymentId, deploymentId));
		wfLowcodeService.dropTable(list);

		// 删除部署表单
		wfDeploymentFormMapper.delete(new LambdaQueryWrapper<WfDeploymentForm>()
				.eq(WfDeploymentForm::getDeploymentId, deploymentId));
	}
