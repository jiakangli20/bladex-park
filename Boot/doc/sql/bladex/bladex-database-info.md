...
Table: blade_auth_lock（认证锁定记录表）

| Field           | Type         | Null | Key | Default | Remarks                                   |
| --------------- | ------------ | ---- | --- | ------- | ----------------------------------------- |
| id              | BIGINT(20)   | NO   | PRI |         | 主键                                        |
| user_id         | BIGINT(20)   | YES  |     |         | 关联用户ID                                  |
| tenant_id       | VARCHAR(12)  | YES  |     | 000000  | 租户ID                                      |
| lock_type       | VARCHAR(20)  | NO   |     |         | 锁定类型（SYSTEM-系统自动/MANUAL-人工手动）  |
| lock_status     | VARCHAR(20)  | NO   |     |         | 锁定状态（PRE_LOCKED-待锁定/LOCKED-锁定中/UN_LOCKED-已解锁） |
| lock_target     | VARCHAR(100) | NO   |     |         | 锁定目标（账号名或IP地址）                   |
| remote_ip       | VARCHAR(50)  | YES  |     |         | 请求IP                                  |
| user_agent      | VARCHAR(500) | YES  |     |         | 用户代理                                    |
| fail_count      | INT(10)      | YES  |     | 0       | 认证失败次数                                |
| lock_begin_time | DATETIME(19) | NO   |     |         | 锁定开始时间                                |
| lock_end_time   | DATETIME(19) | YES  |     |         | 锁定结束时间（MANUAL类型为空表示永久锁定，SYSTEM-LOCKED状态为锁定到期时间） |
| lock_reason     | VARCHAR(500) | YES  |     |         | 锁定原因                                    |
| unlock_reason   | VARCHAR(500) | YES  |     |         | 解锁原因                                    |
| status          | INT(10)      | YES  |     | 1       | 状态                                        |
| is_deleted      | INT(10)      | YES  |     | 0       | 是否已删除                                  |

Table: blade_api_key（令牌权限表）
...
| name        | VARCHAR(128) | NO   |     |         | 密钥名称     |
| api_key     | VARCHAR(128) | NO   |     |         | API Key      |
| api_path    | TEXT(65535)  | YES  |     |         | 访问权限     |
| expire_time | DATETIME(19) | YES  |     |         | 过期时间     |
| ext_params  | TEXT(65535)  | YES  |     |         | 扩展参数(JSON)|
| create_user | BIGINT(20)   | YES  |     |         | 创建人       |
| create_dept | BIGINT(20)   | YES  |     |         | 创建部门     |
| create_time | DATETIME(19) | YES  |     |         | 创建时间     |
| update_user | BIGINT(20)   | YES  |     |         | 修改人       |
...
Table: blade_api_key_log（令牌调用日志表）
...
| api_key_id  | BIGINT(20)    | YES  |     |                   | API Key主键ID   |
...
| create_time | DATETIME(19)  | YES  |     | NULL              | 创建时间        |
...
Table: blade_code_setting（代码生成器配置表）

| Field      | Type        | Null | Key | Default | Remarks |
|------------|-------------| ---- | --- |---------|---------|
| id         | BIGINT(20)  | NO   | PRI |         | 主键      |
| name       | VARCHAR(32) | YES  |     |         | 名称      |
| code       | VARCHAR(16) | YES  |     |         | 编号      |
| category   | INT(10)     | YES  |     | 1       | 分类       |
| settings   | LONGTEXT    | YES  |     |         | 配置项     |
| status     | INT(10)     | YES  |     |         | 状态      |
| is_deleted | INT(10)     | YES  |     | 0       | 是否已删除   |
...
| leader_id     | VARCHAR(1000) | YES  |     |         | 主管ID    |
...
| name          | VARCHAR(128)  | YES  |     |         | 区划名称    |
| province_code | VARCHAR(12)  | YES  |     |         | 省级区划编号  |
| province_name | VARCHAR(128)  | YES  |     |         | 省级名称    |
| city_code     | VARCHAR(12)  | YES  |     |         | 市级区划编号  |
| city_name     | VARCHAR(128)  | YES  |     |         | 市级名称    |
| district_code | VARCHAR(12)  | YES  |     |         | 区级区划编号  |
| district_name | VARCHAR(128)  | YES  |     |         | 区级名称    |
| town_code     | VARCHAR(12)  | YES  |     |         | 镇级区划编号  |
| town_name     | VARCHAR(128)  | YES  |     |         | 镇级名称    |
| village_code  | VARCHAR(12)  | YES  |     |         | 村级区划编号  |
| village_name  | VARCHAR(128)  | YES  |     |         | 村级名称    |
...
Table: blade_tenant_datasource（多租户数据源表）
...
| path        | VARCHAR(255) | YES  |     |         | 顶部菜单路由  |
| sort        | INT(10)      | YES  |     |         | 顶部菜单排序  |
| is_main     | INT(10)      | YES  |     | 0       | 是否主页    |
| create_user | BIGINT(20)   | YES  |     |         | 创建人     |
| create_dept | BIGINT(20)   | YES  |     |         | 创建部门    |
| create_time | DATETIME(19) | YES  |     |         | 创建时间    |
| update_user | BIGINT(20)   | YES  |     |         | 修改人     |
...
| server_ip      | VARCHAR(255)  | YES  |     |         | 服务器IP地址 |
| env            | VARCHAR(255)  | YES  |     |         | 服务器环境   |
| record_level   | VARCHAR(12)   | YES  |     |         | 审计级别    |
| method         | VARCHAR(10)   | YES  |     |         | 操作方式    |
| request_uri    | VARCHAR(255)  | YES  |     |         | 请求URI   |
| user_agent     | VARCHAR(1000) | YES  |     |         | 用户代理    |
| remote_ip      | VARCHAR(255)  | YES  |     |         | 操作IP地址  |
| operation      | VARCHAR(255)  | YES  |     |         | 操作类型    |
| table_name     | VARCHAR(255)  | YES  |     |         | 数据表名    |
| old_data       | TEXT(65535)   | YES  |     |         | 操作前参数   |
| new_data       | TEXT(65535)   | YES  |     |         | 操作后参数   |
| record_message | TEXT(65535)   | YES  |     |         | 审计消息    |
| record_result  | TEXT(65535)   | YES  |     |         | 审计结果    |
| record_cost    | VARCHAR(64)   | YES  |     |         | 记录耗时    |
| record_time    | DATETIME(19)  | YES  |     |         | 记录时间    |
| record_user    | BIGINT(20)    | YES  |     |         | 记录人     |
| status         | INT(10)       | YES  |     | 1       | 状态      |
| is_deleted     | INT(10)       | YES  |     | 0       | 是否已删除   |
