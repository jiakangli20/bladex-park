...
/**
 * 在 option 配置中查找指定 prop 的 column 对象
 * 支持两种格式：
 * 1. column 数组格式：[{ prop: 'xxx', ... }, ...]
 * 2. group 数组格式：[{ column: [...] }, ...] - group 中包含 column 数组
 * @param {Array} arr - column 数组或 group 数组
 * @param {string} prop - 要查找的 column 的 prop 值
 * @returns {Object|null} 返回找到的 column 对象引用，未找到返回 null
 */
export const findColumn = (arr, prop) => {
  ...
  // 使用 flatMap 统一处理两种格式：
  // - group 格式：item.column 存在，展开为 column 数组
