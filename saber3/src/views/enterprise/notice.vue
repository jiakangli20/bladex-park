  ...
  <basic-container>
    <div class="notice-page">
      ...
      <section class="notice-table-card">
        <div class="notice-toolbar"><el-button v-if="permissionList.addBtn" type="primary" icon="el-icon-plus" @click="openAdd">新增公告</el-button><el-tooltip content="刷新"><el-button icon="el-icon-refresh" circle @click="refreshChange" /></el-tooltip></div>
        <el-table v-loading="loading" :data="data" border row-key="id" class="notice-table">
...
<script>
import { getList, update, add, getNotice } from '@/api/desk/notice';
...
export default {
  ...
  methods: {
    ...
    openAdd() { this.form = {}; this.viewMode = false; this.formVisible = true; },
    openEdit(row) { this.viewMode = false; getNotice(row.id).then(res => { this.form = res.data.data || row; this.formVisible = true; }); },
    openView(row) { this.viewMode = true; getNotice(row.id).then(res => { this.form = res.data.data || row; this.formVisible = true; }); },
    submitNotice() { if (!this.form.title || !this.form.releaseTime) { this.$message.warning('请填写公告标题和发布日期'); return; } this.saving = true; const action = this.form.id ? update : add; action(this.form).then(() => { this.$message.success('操作成功'); this.formVisible = false; this.onLoad(this.page); }).finally(() => { this.saving = false; }); },
    searchReset() {
      this.query = {};
