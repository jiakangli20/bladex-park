...
import { getList, getDetail, remove, submit } from '@/api/ics/building';
import { getList as getParkList } from '@/api/park/park';
import { tableOption } from '@/option/ics/building';
...
export default {
        ...
        column: [
          {
            ...
            action: '/blade-ics/building/import',
          },
          {
            label: '模板下载',
            prop: 'excelTemplate',
  ...
  methods: {
    ...
    handleExport() {
      exportBlob('/blade-ics/building/export', this.query).then(res => {
        downloadXls(res.data, `建筑数据${this.$dayjs().format('YYYY-MM-DD HH:mm:ss')}.xlsx`);
      });
    },
    handleTemplate() {
      exportBlob(
        `/blade-ics/building/export-template?${this.website.tokenHeader}=${getToken()}`
      ).then(res => {
